...
> **Stop 那条的 15 秒只是够用、不是靠它保证收拾干净。** 收工分两步：先在状态锁里一笔
> 关账（清空活动窗口**之前**把「哪扇窗还没收」追加到 `cleanup_queue`），再 best-effort
> 地有界等推送锁去即时删窗/定格。万一在飞的慢请求握着推送锁、宿主又把 Stop 超时杀了，
> 清理责任已经落盘——**下一轮推送或下一次 Stop** 持同一把推送锁时会补做，那扇窗不会因为
> 这次没收成就永远留在聊天里。所以调大 Stop 的 timeout 只是让「当场收拾」更常发生，
> **不是**收拾干净的前提。
> 队列保留每一轮尚未完成的任务；新一轮重建状态时也会保留它。成功清理后只移除对应任务，
> 网络错误、429 限流、5xx 或不能确定为永久失效的错误会留给后续执行者重试；只有明确的
> 消息永久不可编辑等错误才结束重试。旧版本的 `pending_cleanup` 单条任务会自动读入队列。
> 升级时先退出旧版本 hook 进程，避免旧代码重建状态时丢掉它不认识的新队列。
> 如果此后没有新的 push 或 Stop，任务会保持待处理，不承诺后台定时执行。
...
> 共用库的并发写靠一把**系统锁**（Unix `flock` / Windows `msvcrt.locking`）串起来。
> 同一 `file_unique_id` 的 pending 发布、认领读取、记录更新和清理共用一把锁；不同贴纸
> 可以并行下载，库编号分配与提交再使用库锁。pending 和最终原图都通过独立临时文件原子发布，
> 认领不会读取到另一调用写了一半的原图，清理也不会删除另一调用刚发布的待认领记录。
> ⚠️ **「跨平台」只保证两端各自在本地文件系统上运行**：Linux 一群进程 / Windows 一群
> 进程各自安全。**Windows 与 WSL 同时写同一个共享目录**、或把库放在网络盘/云同步目录，
> 是两套锁协议或不保证锁语义的文件系统混跑——**未覆盖，需另行验证**，别默认它互斥得住。
> 升级时旧版写入进程要全部退出：旧版的「mtime 超龄夺锁」会把新版常驻的锁文件当死锁删掉，
> 两套协议不能混跑（换锁文件名也不解决）。
...
5. 🔴 **复制会塌，转发不会**（用户实测）：结构是客户端渲染的，剪贴板只接得住
   **纯文本那一层**，粘出来是塌平的一长条（表格变行、折叠块摊开、公式变裸 LaTeX、
   ...
   还有一条：作者实测里，用户**转发富消息给 bot 时，agent 读到的只有纯文本**——
   但那是**当时那个入站插件**的行为，不是 Bot API 的通用事实（官方 `Message`
   对象本身有 `rich_message` 字段，透不透给 agent 取决于收信侧管道，见 COOKBOOK §7）。
...
17. **组合发送半途失败 ⇒ 重复消息**：先发文字段、再发文件段（或多个文字段）的
    ...
    **机器可读的分段送达账**，附在人类文案后的 `分段送达账（机器可读）：` 之后，
    是一个 `{"delivered":[…],"failed":[…],"unknown":[…],"not_attempted":[…]}` 的 JSON。
    **每个计划段都恰好落进一个桶**，四类各有各的恢复方式：
    - `delivered`：已确认送达的段。文字段带 `message_id`；贴纸段也带 `message_id`。
      **发送后的缓存维护失败不推翻送达**——贴纸已送到、只是 `file_id` 没缓存上时，
      仍记 delivered，另挂 `cache_warning`（下次可能再上传一遍，不影响这条已送达）。
      恢复方式：**什么都不用做**，它已经到了。
    - `failed`：服务器**明确拒收**（`ok:false`，如格式不支持），或发送前的本地预检
      失败（贴纸归档不在库/格式不对/原图丢失）——**肯定没发出去**。
      恢复方式：**可安全补发这一段**（贴纸段先解决本地问题）。
    - `unknown`：**超时/网络错**的段，请求可能已被 Telegram 收到，**送达状态未知**。
      恢复方式：**不得当成没发出去自动补发**；先想办法确认是否送达，再决定补不补。
    - `not_attempted`：中断时**尚未遍历到的尾段**（从没试过发，与「发了没拿到回执」的
      unknown 是两回事）。恢复方式：**接着从这些段发下去即可**。
    文字段与贴纸段用**同一套**状态分类（网络错都进 unknown、服务器拒收都进 failed），
    别对两者用不同口径。后段失败时前段的 `delivered`（含 message_id）**一定保留在错误
    结果里**，整条被标 `isError` 但绝不自动整条重发；要补只补 `failed`/`not_attempted`
    里点名的那几段（`unknown` 先确认送达再说），别把 `delivered` 的再发一遍。
