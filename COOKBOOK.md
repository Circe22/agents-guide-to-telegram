...
3. **file_id 是「这个 bot」的资产**。多个 bot 共用一套库时，共享层放原图 +
   ...
   缓存在自己名下（懒迁移，不用手工逐张重传）。**只有 Telegram 明确回 400 才算
   ID 失效**、才触发重传；网络错/限流/5xx 都不算——判错了会重复上传。
...
反查的料：`file-ids.<bot_id>.json` 有 file_id→unique，`library.json` 有
unique→title，两跳就回到标题）；收信侧拿到反应通知时按 id 查表，把通知写成
「❤ → 贴纸『敲键盘炸毛猫』」。
